...
const darkColorMap2: DarkColorMap2 = {
...
export function colorPalette(color: string, index: number) {
  ...
  // 浅色部分
  if (index <= lightColorCount) {
    i = lightColorCount - index + 1;
    const LightColorString: string = toHex(
      inputToRGB({
        h: getHue(hsv, i, true),
        s: getSaturation(hsv, i, true),
        v: getValue(hsv, i, true),
...
export function darkColorPalette(color: string, index: number, backgroundColor?: string) {
  const { index: colorIndex, opacity } = darkColorMap2[index];
  const lightColor = colorPalette(color, colorIndex + 1);
  const darkColorString: string = toHex(
    mix(
      inputToRGB(backgroundColor || '#141414'),
      inputToRGB(lightColor),
      opacity * 100,
    ),
  );
  return darkColorString;
...
export function generateColor(color: string, opts: PaletteOptions) {
  const { dark, index = 6, backgroundColor = '#141414' } = opts;

  return dark ? darkColorPalette(color, index, backgroundColor) : colorPalette(color, index);
}
